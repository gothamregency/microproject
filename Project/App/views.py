...
def timeNum(request, time_num):
    if request.method == "POST":
        if 'duration' in request.POST:
            select = request.POST['duration']
            if select == "01":
                return int(time_num)
            elif select == "02":
                return int(time_num) * 7
            else:
                return 1
        else:
            messages.error(request, "no select")
            return 1
    else:
        return 1
...
def accomodate(request):
    ...
    if request.method == "POST":
        if 'accomodations' in request.POST:
            select = request.POST['accomodations']
            if select in myLis:
                return int(myLis[myLis.index(select)])
            else:
                return 1
        else:
            return 1
...
def find(request):
    ...
    if request.method == "POST":
        ...
        if not request.user.is_authenticated:
            messages.error(request, "Can't book, please login")
            return redirect("/login")
            ...
            try:
                request.user.credit_points -= (int(timeNum(request, request.POST['duration_num'])) * accomodate(request) * 1000)
                request.user.save()
                messages.success(request, "Booked hotel  successfully")
            except KeyError as k:
                messages.error(request, "Please fill the entire form")
                return redirect("/find")
            except ValueError:
                messages.error(request, "Val error")
                return redirect("/find")
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if button == "gold":
                ...
                request.user.save()
                messages.success(request, "Added $10,000 credits")
